...
class TMSiDongleList(Structure):
    ...
    _fields_ = [
        (
            "TMSiDongleID",
            c_ushort,
        ),  # Unique ID to identify a bluetooth-dongle, used for pairing and connect purposes.
...
class TMSiDevInfoReport(Structure):
    ...
    _fields_ = [
        ...
        (
            "FWBTPCVersion",
            c_ushort,
        ),  # The current firmware version 0xUU.LL of the PC Bluetooth module
        (
            "FWBTPCVersionString",
            c_char * 18,
        ),  # The current firmware version string of the PC Bluetooth module
        (
            "FWBTSensorVersion",
            c_ushort,
        ),  # The current firmware version 0xUU.LL of the Sensor Bluetooth module
        (
            "FWBTSensorVersionString",
            c_char * 18,
        ),  # The current firmware version string of the Sensor Bluetooth module
        (
            "NrOfHWChannels",
            c_ushort,
        ),  # Total number of hardware channels (ExG, BIP, AUX).
        ...
        (
            "DeviceName",
            c_char * 18,
        ),  # Full device name 17 char string (zero terminated).
    ]
...
class TMSiDevSamplingCfg(Structure):
    ...
    _fields_ = [
        ...
        (
            "LiveImpedance",
            c_byte,
        ),  # <ControlDisabled> = No Imp during measurement reference, <ControlEnabled> = LiveIMp.
    ]
...
class TMSiDevChName(Structure):
    ...
    _fields_ = [
        (
            "ChanIdx",
            c_ushort,
        ),  # Which channel, in the channel-array, is this configure for.
...
class TMSiDevChanRef(Structure):
    ...
    _fields_ = [
        ...
        (
            "ChanRefStatus",
            c_byte,
        ),  # -1 Channel disabled, 0 Channel not in reference, 1 Channel in reference
    ]
...
class TMSiChannelMetadata(Structure):
    ...
    _fields_ = [
        ...
        (
            "ChannelType",
            c_ushort,
        ),  # 0=Unknown, 1=UNI, 2=BIP, 3=AUX, 4=DIGRAW/Sensor, 5=STATUS, 6=COUNTER, 7=IMP, 8=Cycl
        (
            "ChannelFormat",
            c_ushort,
        ),  # 0x11xx Float, 0x00xx Unsigned xx bits, 0x01xx signed xx bits
...
class TMSiImpedanceMetadata(Structure):
    ...
    _fields_ = [
        (
            "ChanIdx",
            c_ushort,
        ),  # The channel index number. (starts at 0 for channel 1)..
        ("ChanName", c_char * 10),  # Default channel name 9 char zero terminated.
        (
            "ImpedanceReUnit",
            c_char * 10,
        ),  # Impedance Unit name , real part, 9 char zero terminated.
        (
            "ImpedanceImUnit",
            c_char * 10,
        ),  # Impedance Unit name, imaginary part, 9 char zero terminated.
    ]
...
class TMSiDevImpedanceRequest(Structure):
    ...
    _fields_ = [
        (
            "StartStop",
            c_byte,
        ),  # Flag to start <StartImpedance> and stop <StopImpedance>
    ]
...
class TMSiEvent(Structure):
    ...
    _fields_ = [
        ...
        (
            "TMSiDeviceID",
            c_ushort,
        ),  # Unique ID to identify device, retrieved with TMSi_GetDeviceList().
        (
            "TMSiDongleID",
            c_ushort,
        ),  # Unique ID to identify a bluetooth-dongle, retrieved with TMSi_GetDongleList().
    ]
