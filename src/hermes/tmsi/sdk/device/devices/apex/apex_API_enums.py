...
class TMSiDeviceRetVal(Enum):
    ...
    TMSiStatusDrChecksumError = (
        0x01010001  # DR reports "Checksum error in received block"
    )
    ...
    TMSiStatusDrInterfaceAlreadyOpen = (
        0x01010004  # DR reports "Interface is already open"
    )
    TMSiStatusDrUnknownCommandForInterface = (
        0x01010005  # DR reports "Command not supported over current interface"
    )
    TMSiStatusDrDeviceRecording = (
        0x01010006  # DR reports "Command not possible, device is recording"
    )
    ...
    TMSiStatusDrServiceModeLocked = (
        0x01010009  # DR reports "Device service mode is locked"
    )

    # Additional defines below for DLL error types.
    TMSiStatusDllDeviceNotAvailable = 0x03001001  # DLL reports "Device not available".
    TMSiStatusDllInterfaceAlreadyOpen = (
        0x03001002  # DLL reports that interface is already opened.
    )
    TMSiStatusDllDeviceNotPaired = 0x03001003  # DLL reports that it is not paired
    TMSiStatusDllDeviceAlreadyPaired = (
        0x03001004  # DLL reports that it is already paired.
    )
    TMSiStatusDllNotImplemented = (
        0x03001005  # DLL Function is declared, but not yet implemented
    )
    TMSiStatusDllInvalidParameter = (
        0x03001006  # DLL Function called with invalid parameters
    )
    TMSiStatusDllChecksumError = 0x03001007
    TMSiStatusDllInternalError = (
        0x03001008  # Function failed because an underlying process failed
    )
    TMSiStatusDllBufferError = 0x03001009  # Function called with a too small buffer
    TMSiStatusDllInvalidHandle = (
        0x0300100A  # Function called with a Handle that's not assigned to a device
    )
