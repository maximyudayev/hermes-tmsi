...
class SignalMeasurement(TMSiMeasurement):
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>APEX-SDK: set device sampling request ON".format(self.get_name())
        )
    ...
    def stop(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>APEX-SDK: set device sampling request OFF".format(self.get_name())
        )
        self._dev.set_device_sampling_request(measurement_request)
    ...
    def _sampling_function(self):
        TMSiLoggerActivity().log(
            "Sampling Thread->>APEX-SDK: GET signal samples request"
        )
        ...
        TMSiLoggerActivity().log(
            "APEX-SDK->>Sampling Thread: GET signal samples response"
        )
        if ret == TMSiDeviceRetVal.TMSiStatusOK:
            ...
            TMSiLoggerActivity().log(
                "APEX-SDK->>Sampling Thread: negative response: {}".format(ret)
            )
