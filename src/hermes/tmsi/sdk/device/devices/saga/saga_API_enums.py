...
class TMSiDeviceRetVal(Enum):
    ...
    TMSI_DR_CONFIGURATION_ERROR = 0x01010008  # DR reported 'Configuration error".
    TMSI_DR_COMMAND_NOT_SUPPORTED_BY_CHANNEL = (
        0x01030001  # DR reported "Command not supported by Channel".
    )
    TMSI_DR_AMBREC_ILLEGAL_START_CTRL = (
    ...
    TMSI_DS_COMMAND_NOT_SUPPORTED = (
        0x02010005  # DS reported "Command not supported over current interface".
    )
    TMSI_DS_UNEXPECTED_RESPONSE = 0x02010006  # DS reported "Unexpected response".
    ...
    TMSI_DLL_INVALID_HANDLE = (
        0x03001006  # DLL Function called with a Handle that's not assigned to a device
    )
