...
class SignalMeasurement(TMSiMeasurement):
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>SAGA-SDK: set device sampling request ON".format(self.get_name())
        )
    ...
    def stop(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>SAGA-SDK: set device sampling request OFF".format(self.get_name())
        )
        self._dev.set_device_sampling_request(measurement_request)
        ...
        while not self._conversion_queue.empty():
            ...
            if retrieved_sample_sets > 0:
                ...
                for j in range(len(self._masked_channels)):
                    sample_mat[self._masked_channels[j]] = self._mask_functions[j](
                        sample_mat[self._masked_channels[j]]
                    )
    ...
    def _sampling_function(self):
        TMSiLoggerActivity().log(
            "Sampling Thread->>SAGA-SDK: GET signal samples request"
        )
        ...
        TMSiLoggerActivity().log(
            "SAGA-SDK->>Sampling Thread: GET signal samples response"
        )
