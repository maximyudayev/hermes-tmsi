...
class ImpedanceMeasurement(TMSiMeasurement):
    ...
    def __init__(self, dev, name="Impedance Measurement"):
        ...
        TMSiLoggerActivity().log(
            "TMSi-SDK->>{}: create measurement".format(self.get_name())
        )
        TMSiLoggerActivity().log(
            "{}->>{}-SDK: GET device channels request".format(
                self.get_name(), self._dev.get_device_type()
            )
        )
        channels = self._dev.get_device_active_impedance_channels()
        TMSiLoggerActivity().log(
            "{}-SDK->>{}: GET device channels response".format(
                self._dev.get_device_type(), self.get_name()
            )
        )
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>SAGA-SDK: set device impedance request ON".format(self.get_name())
        )
    ...
    def stop(self):
        ...
        TMSiLoggerActivity().log(
            "{}->>SAGA-SDK: set device impedance request OFF".format(self.get_name())
        )
        self._dev.set_device_impedance_request(measurement_request)
    ...
    def _conversion_function(self):
        while not self._conversion_queue.empty():
            ...
            TMSiLoggerActivity().log(
                "Conversion Queue->>Conversion Thread: GET samples | package size:{} - new size:{}".format(
    ...
    def _sampling_function(self):
        TMSiLoggerActivity().log(
            "Sampling Thread->>SAGA-SDK: GET impedance samples request"
        )
        ...
        TMSiLoggerActivity().log(
            "SAGA-SDK->>Sampling Thread: GET impedance samples response"
        )
        if ret == TMSiDeviceRetVal.TMSI_OK:
            TMSiLoggerActivity().log(
                "Sampling Thread->>Sampling Thread: positive response"
            )
            if self._retrieved_sample_sets.value > 0:
                ...
                TMSiLoggerActivity().log(
                    "Sampling Thread->>Conversion Queue: PUT samples | package size:{} - new size:{}".format(
                        self._retrieved_sample_sets.value,
                        self._conversion_queue.qsize(),
                    )
                )
                ...
                TMSiLoggerActivity().log(
                    "Sampling Thread->>Sampling Thread: empty package"
                )
                if self._empty_read_counter == 0:
                    self._empty_read_counter = 1
        else:
            TMSiLoggerActivity().log(
                "SAGA-SDK->>Sampling Thread: negative response: {}".format(ret)
            )
        if self._download_samples_limit is not None:
            ...
            TMSiLoggerActivity().log(
                "Sampling Thread->>Sampling Thread: downloaded {} samples of {}".format(
                    self._downloaded_samples, self._download_samples_limit
                )
            )
