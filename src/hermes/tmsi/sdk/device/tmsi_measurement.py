...
class TMSiMeasurement:
    ...
    def __init__(self, dev, name="TMSi Measurement"):
        ...
        for i in range(len(channels)):
            if (
                channels[i].get_channel_format() == 0x0020
                or channels[i].get_channel_format() == 0x0120
            ):
                self._float_channels.append(i)
            else:
                if (channels[i].get_channel_type().value == 3) and (
                    channels[i].get_sensor_information() != None
                ):
        ...
        self._sampling_thread = TMSiThread(
            name="Sampling Thread", looping_function=self._sampling_function, pause=0.01
        )
